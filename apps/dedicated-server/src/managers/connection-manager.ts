// apps/dedicated-server/src/managers/connection-manager.ts
...
export class ConnectionManager {
    ...
    private async handleLeaveTable(socket: Socket): Promise<void> {
        try {
            const connection = this.connections.get(socket.id);
            ...
            const result = await this.tableManager.removePlayerFromTable(connection.tableId, connection.playerId);

            if (result.success) {
                // Leave socket room
                socket.leave(connection.tableId);

                // Notify other players
                socket.to(connection.tableId).emit('table:player-left', {
                ...
                logger.info(`Player ${connection.playerId} left table`);
            } else {
                socket.emit('error', { code: 'LEAVE_TABLE_FAILED', message: result.error });
            }
        } catch (error) {
            logger.error('Error leaving table:', error);
            socket.emit('error', { code: 'INTERNAL_ERROR', message: 'Failed to leave table' });
        }
    }

    private async handleReconnect(socket: Socket, data: { reconnectToken: string }): Promise<void> {
        try {
            // Validate reconnect token and restore session
            const sessionInfo = await this.validateReconnectToken(data.reconnectToken);

            if (!sessionInfo) {
                socket.emit('error', { code: 'INVALID_RECONNECT_TOKEN', message: 'Invalid or expired reconnect token' });
                return;
            }
            ...
            this.playerSockets.set(sessionInfo.playerId, socket);

            // Rejoin table if applicable
            if (sessionInfo.tableId) {
                socket.join(sessionInfo.tableId);

                // Notify table of reconnection
                socket.to(sessionInfo.tableId).emit('player:reconnected', {
                    playerId: sessionInfo.playerId
                });

                // Send current game state
                const gameState = await this.gameManager.getGameState(sessionInfo.tableId);
                if (gameState) {
                    socket.emit('game:state-sync', { fullState: gameState });
                }
            }
            ...
            logger.info(`Player ${sessionInfo.playerId} reconnected successfully`);
        } catch (error) {
            logger.error('Error during reconnection:', error);
            socket.emit('error', { code: 'RECONNECTION_FAILED', message: 'Failed to reconnect' });
        }
    }

    private async handleGameAction(socket: Socket, data: { action: PlayerAction }): Promise<void> {
        try {
            const connection = this.connections.get(socket.id);
            if (!connection || !connection.tableId) {
                socket.emit('error', { code: 'PLAYER_NOT_AT_TABLE', message: 'Not seated at any table' });
    ...
    private async handleGameReady(socket: Socket): Promise<void> {
        try {
            const connection = this.connections.get(socket.id);
            if (!connection || !connection.tableId) {
                socket.emit('error', { code: 'PLAYER_NOT_AT_TABLE', message: 'Not seated at any table' });
            ...
            await this.gameManager.setPlayerReady(connection.tableId, connection.playerId);

            // Notify table
            socket.to(connection.tableId).emit('notification', {
                type: 'info',
                message: 'Player ready'
            });

            logger.debug(`Player ${connection.playerId} is ready`);
        } catch (error) {
            logger.error('Error setting player ready:', error);
            socket.emit('error', { code: 'READY_FAILED', message: 'Failed to set ready state' });
        }
    }
    ...
    private async validateReconnectToken(token: string): Promise<{
        playerId: string;
        tableId: string | null;
        position: number | null;
    } | null> {
        // This would typically validate JWT token and get session info
        // For now, return null to indicate invalid token
        // In real implementation, decode JWT and verify with master server
        return null;
    }
    ...
    private scheduleConnectionCleanup(socketId: string, connection: PlayerConnection): void {
        setTimeout(() => {
    ...
    getConnectionStats(): any {
        return {
            ...
            securityMetrics: this.securityManager ? this.securityManager.getMetrics() : null
        };
    }
}